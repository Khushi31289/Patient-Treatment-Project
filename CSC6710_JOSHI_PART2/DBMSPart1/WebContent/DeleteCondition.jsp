...
<title>Delete a condition for a patient</title>
<style type="text/css">
	div{
		padding-top: 15px;
	}
...
function GetPConditionByAjax(){
    var valPid = $('#selPid').val();
   // alert(valpid);
    if(valPid > 0){
	    $.get('GetPatientConditionByAjax', {
	            Pid : valPid
...
<body id="myBody" onload="">
	<h1>Delete Condition for a Patient</h1>
	<form action="DeletePatientConditions" method="post" id="frmDelete"> 
		<div>
			<label>Symptom Id<sup>*</sup>:</label>
			&nbsp;
			<select id = "selPid" name = "selPid" required="required" onchange="GetPConditionByAjax();"> 
