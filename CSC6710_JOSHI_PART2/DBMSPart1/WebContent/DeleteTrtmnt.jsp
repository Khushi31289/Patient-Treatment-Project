...
<title>Delete a Treatment for a patient</title>
<style type="text/css">
	div{
		padding-top: 15px;
	}
...
function GetTreatmentsByAjax(){
    var valPid = $('#selPid').val();
   // alert(valpid);
    if(valPid > 0){
	    $.get('GetTreatmentByAjax', {
	            Pid : valPid
...
<body id="myBody" onload="">
	<h1>Delete Treatment for a Patient</h1>
	<form action="DeleteTreatment" method="post" id="frmDelete"> 
		<div>
			<label>Treatment Id<sup>*</sup>:</label>
			&nbsp;
			<select id = "selPid" name = "selPid" required="required" onchange="GetTreatmentsByAjax();"> 
		...
		<div>
			<label>Treatment Name:<sup>*</sup>:</label>
			&nbsp;<input type="text" name="txtName" id="txtName" required="required" 
			maxlength="255" value="${name}" readonly="readonly" ><br>
		</div>
		<div>
			...
			<select id = "selStatus" name = "selStatus" required="required" disabled="disabled"> 
	...
	<p align="center"><b><i>${deleteStatus}</i></b></p>
</body>
</html>
