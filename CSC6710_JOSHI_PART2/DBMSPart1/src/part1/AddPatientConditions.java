 ...
 */
@WebServlet("/AddPatientConditions")
public class AddPatientConditions extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
									throws ServletException, IOException {
		addStatus = TextBank.ENTRDATA_PATIENTCONDITION;
		//TextBank.UPDATE_PATIENTS_QUERY = "UPDATE patients SET ";
		TextBank.INSERT_PATIENTCON_QUERY = "INSERT INTO PatientCondition(pconname, pid,pconstatus) VALUES";
		...
		request.getRequestDispatcher("/AddCondition.jsp").forward(request, response);
	}
		...
		if(valPid != null && valPid != "0" ){
			...
			if(valName !="" && valName !=null && 
				...
				if(Integer.parseInt(valPid) > 0){
					...
					TextBank.INSERT_PATIENTCON_QUERY += "('" + valName + "' , "+
													str + ", '"
													+ valStatus + "')";
					System.out.println(TextBank.INSERT_PATIENTCON_QUERY);
					insertValues(req);
				}else{
					addStatus = TextBank.ENTRDATA_PATIENTCONDITION;
				}
	...
	private void insertValues(HttpServletRequest req) {
		...
		if(driver ==  true){
				...
				try {
					MyJDBCDriver .getConn();
					MyJDBCDriver .getStatement();
					MyJDBCDriver.getResultSetForExecuteUpdate(TextBank.INSERT_PATIENTCON_QUERY );
					if(MyJDBCDriver.queryOP >= 1 ){
						addStatus = TextBank.ABLE_TO_INSERT_PATIENTCONDITION;
					}
				} catch (Exception e) {
					GetErrorMsg.printError(e);
					addStatus = TextBank.UNABLE_TO_INSERT_PATIENTCONDITION;
				}finally{
						MyJDBCDriver.closeAll();
				}
			}
