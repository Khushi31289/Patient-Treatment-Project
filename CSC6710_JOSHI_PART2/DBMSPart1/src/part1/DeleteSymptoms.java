...
@WebServlet("/DeleteSymptoms")
public class DeleteSymptoms extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		TextBank.DELETE_SYMPTOM_QUERY = "DELETE FROM symptom";
		valSympId = "";
		...
		request.getRequestDispatcher("/DeleteSymptom.jsp").forward(request, response);
	}
	...
	private void getFormData(HttpServletRequest req) {
		if(valSympId != null && valSympId != "0" ){
			...
			System.out.println("Here 2 " + TextBank.DELETEDATA_SYMPTOM );
			//if(valName !="" && valName !=null && 
				...
				if(Integer.parseInt(valSympId) > 0){
					...
					TextBank.DELETE_SYMPTOM_QUERY += TextBank._WHERE +	"sympid = " + str
							+ TextBank._AND + "pid = "+ Integer.parseInt(valPid);
					System.out.println(TextBank.DELETE_SYMPTOM_QUERY);
					deleteValues(req);
				}else{
					deleteStatus = TextBank.DELETEDATA_SYMPTOM;
				}
	...
	private void deleteValues(HttpServletRequest req) {
		...
		if(driver ==  true){
				...
				try {
					MyJDBCDriver .getConn();
					MyJDBCDriver .getStatement();
					MyJDBCDriver.getResultSetForExecuteUpdate(TextBank.DELETE_SYMPTOM_QUERY);
					if(MyJDBCDriver.queryOP >= 1 ){
						deleteStatus = TextBank.ABLE_TO_DELETE_SYMPTOM;
					}
				} catch (Exception e) {
					GetErrorMsg.printError(e);
					deleteStatus = TextBank.UNABLE_TO_DELETE_SYMPTOM;
				}finally{
						MyJDBCDriver.closeAll();
				}
			}
