...
/**
 * Servlet implementation class GetPatientConditionByAjax
 */
@WebServlet("/GetPatientConditionByAjax")
public class GetPatientConditionByAjax extends HttpServlet {
	...
	private void getSymptomIDs(HttpServletRequest req, HttpServletResponse resp, String pid) {
		...
		if(driver ==  true){
				...
				try {
					...
					MyJDBCDriver.getResultSet(TextBank.AJAX_SEL_PATIENTCONDITION_INFO_UPDATE  + 
										" where pconid=" + Integer.parseInt(pid));
					int pID = 0;
	...
	try {
		System.out.println("pid: " + resultSet.getString("pid") + ", pconname: " + 
				resultSet.getString("pconname") + ", status: "+ resultSet.getString("pconstatus"));
		responseString += resultSet.getString("pid") + "<?>"; 
		responseString += resultSet.getString("pconname") + "<?>"; 
		responseString += resultSet.getString("pconstatus")+ "<?>";
