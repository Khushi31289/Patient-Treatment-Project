...
/**
 * Servlet implementation class QueryNo6
 */
@WebServlet("/QueryNo6")
public class QueryNo6 extends HttpServlet {
	...
	private void getNewPatientID(HttpServletRequest req, HttpServletResponse resp) {
		...
		if(driver ==  true){
				...
				try {
					...
					MyJDBCDriver.getResultSet(TextBank.SEARCH_QUERY_6);
					...
					e.printStackTrace();
			        	...
			        	//	System.out.println(i1.next());
			        		int thisPid = i1.next();
			        		boolean found = checkForDiabetesDisappear(thisPid);
			        		if(found == true){
	...
	boolean checkForDiabetesDisappear(int _pid) {
		// TODO Auto-generated method stub
		boolean driver= true;
		boolean found = false;
		try {
				...
				String queryString = TextBank.SEARCH_DIABETES_DISAPPEAR_QUERY;
				...
				System.out.println(queryString);
			        ...
			        if(MyJDBCDriver.resultSet != null){
			        	while(MyJDBCDriver.resultSet.next()){
			        		...
			        		if(status.toLowerCase().equals("disappear")){
			        			found = true;
			        		}else{
			        			found = false;
			        		}
